const AudioPlayer = {
  ...
  async ensureMusiumRunning(tracks) {
    ...
    if (launched) {
      ...
      return await this.checkMusiumAvailable();
    }
    return false;
  },
  ...
  async addToPlaylist(album, trackIndex = null) {
    ...
    Utils.showNotification(`Добавлено ${addedCount} из ${tracksToAdd.length} треков в плейлист`, "success");
  ...
  initUI() {
    ...
    if (this.panelPlayPauseBtn) {
      this.panelPlayPauseBtn.addEventListener("click", () => this.togglePlayPause());
    }
  ...
  async previousTrackMusium() {
    ...
    await this.updateUI();
    if (typeof PlaylistViewer !== "undefined") {
      await PlaylistViewer.updateDisplay();
      await PlaylistViewer.updateProgress();
    }
  },
  ...
  async updateUI() {
    ...
    if (stateResponse.data && typeof stateResponse.data.isPlaying !== "undefined") {
    ...
    if (timeResponse && timeResponse.success) {
      if (this.panelTimeCurrent) {
        this.panelTimeCurrent.textContent = this.formatTime(timeResponse.data.position || 0);
      }
      if (this.panelTimeTotal) {
        this.panelTimeTotal.textContent = this.formatTime(timeResponse.data.duration || 0);
      }
