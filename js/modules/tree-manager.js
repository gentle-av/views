const TreeManager = {
  ...
  async loadTreeData(mediaType = null) {
    ...
    if (mediaType) {
      ...
      this.currentPath =
        mediaType === "video" ? CONFIG.ROOT_PATH : CONFIG.MUSIC_PATH;
    }
    this.treeContainer.innerHTML =
      '<div style="padding: 20px; text-align: center; color: var(--fg3);">Загрузка дерева папок...</div>';
    try {
  ...
  buildTree(videoItems, musicItems) {
    if (!this.treeContainer) return;
    const videoFolders = videoItems.filter(
      (item) => item.isDirectory && !Utils.isHiddenFile(item.name),
    );
    const musicFolders = musicItems.filter(
      (item) => item.isDirectory && !Utils.isHiddenFile(item.name),
    );
  ...
  updateActiveItem(path) {
    ...
    if (leftPanel && leftPanel.classList.contains("visible")) {
      ...
      const activeItem = document.querySelector(
        `.tree-item[data-path="${path}"] .tree-item-content`,
      );
      if (activeItem) {
      ...
      const activeGroup = document.querySelector(
        `.tree-group-header[onclick*="'${this.currentMediaType}'"]`,
      );
      if (activeGroup) {
